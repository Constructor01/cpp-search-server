...
const long double MAGIC_CONST = 1e-6;
...
public:
    ...
    {
        int a = numbers_documents.size();
        if (index > a - 1 || index < 0) { //static_cast<int>(numbers_documents.size())
...
private:
    ...
    QueryWord ParseQueryWord(string text) const
    {
        ...
        if (text.empty() or text[0] == '-' or !IsValidWord(text))
        {
            throw invalid_argument("Error");
        }
