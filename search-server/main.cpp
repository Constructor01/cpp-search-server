...
public:
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) 
    {
        ...
        if (document_id < 0 or documents_.count(document_id) > 0)
        {
            throw invalid_argument("Error");
        }
    ...
    {
        if (index > numbers_documents.size() - 1 or index < 0) {
            throw out_of_range("Error");
            return INVALID_DOCUMENT_ID;
        }

        return numbers_documents.at(index);
    }
...
private:
    ...
    //Начало доп. методов
    bool SplitIntoWordsNoStop(const string& text, vector<string>& words)
    {
        for (const string& word : SplitIntoWords(text))
        {
            if (!IsValidWord(word))
    ...
    bool ParseQueryWord(string text, QueryWord& result) const
    {
        ...
        if (word.empty() or word[0] == '-' or !IsValidWord(word))
        {
            return false;
        }
